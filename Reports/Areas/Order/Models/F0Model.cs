...
namespace MvcApplication2.Areas.Order.Models
{
        ...
        public F0Model(String sessionId) // конструктор для захода с другой страницы или из меню
        {
            ...
            RequestPackage rqp = new RequestPackage
            {
                ...
                Parameters = new RequestParameter[] {
                    new RequestParameter { Name = "pattern", Value = @"^order_f0_filter\..*" }
                }
            };
            ...
            if (rsp.Data != null && rsp.Data.Tables.Count > 0)
            {
                ...
                for (; ri < dt.Rows.Count; ri++)
                {
                    ...
                    {
                        name = name.Replace("order_f0_filter.", "");
                ...
                if (!existsParameterType)
                {
                    ...
                    rqp.Parameters[ri] = new RequestParameter { Name = "type", Value = "2" };
                }
            }
            ApplyFilter(rqp);
        }
    ...
    public static class F0Command
    {
        private static void SaveAuctionInf(Guid sessionId, Guid aUid, Int32 specHeadType)
        {
            ...
            Int32 headType = 2; // 1 - заявка, 2 - спецификация
            HomeData.Order.AddSpec(sessionId, aUid, headType);
        ...
        {
            HomeData.Order.AddContractDirectory(sessionId, oUid);
        }
        public static Boolean ExistsAuctionInf(Guid sessionId, String auctionNumber)
        {
            Boolean r = HomeData.Order.ExistsAuctionInf(sessionId, auctionNumber);
            return r;
        }
        ...
        public static void ThreadLoadByNumbers(Object auctionNumbers)
        {
            ...
            {
                LoadAuctionInf(sessionId, an, 2);
                Thread.Sleep(10000);
            }
        }
        public static String PassToTender(Guid sessionId, String auctionNumber)
        {
            String r = HomeData.Order.PassToTender(sessionId, auctionNumber);
            return r;
        }
